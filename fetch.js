...
const URL = 'https://leaderboards.arcaneodyssey.dev/clans';
...
(async () => {
  ...
  await page.waitForSelector('h2, [class*="clan"], [class*="rank"]', { timeout: 15000 }).catch(() => {});

  const clans = await page.evaluate(() => {
    const results = [];
  ...
  if (!clans.length) {
    console.error('No clans found — page may not have rendered correctly.');
    process.exit(1);
  }
  ...
  const snapshots = [...dedupedOlder, ...recentWithoutLast, { ts: now, guilds: clans }];

  fs.writeFileSync(OUT, JSON.stringify({ snapshots }, null, 2));
  console.log(`Wrote ${clans.length} clans. Total snapshots: ${snapshots.length} (${dedupedOlder.length} daily + ${recentWithoutLast.length + 1} recent)`);
})();
